...
export const Morpher = (props) => {
  ...
  const morpher = (start, end) => {
    ...
    const duration = props.duration === undefined ? 4 : props.duration
    // speed of the morph for each letter
    const frameRate = props.frameRate === undefined ? 30 : props.duration
  ...
  if (props.to === undefined) {
    ...
    return (<NavLink to={props.to}>{content()}</NavLink>)
  }
}